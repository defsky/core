...
void BattleGroundAV::EventPlayerDestroyedPoint(BG_AV_Nodes node)
{
    ...
    if (IsTower(node))
    {
        ...
        SendYell2ToAll(LANG_BG_AV_TOWER_TAKEN, LANG_UNIVERSAL, 
                        GetSingleCreatureGuid(BG_AV_HERALD, (ownerTeamIdx == BG_TEAM_ALLIANCE) ? BG_TEAM_HORDE : BG_TEAM_ALLIANCE), 
                        GetNodeName(node), 
...
void BattleGroundAV::ChangeMineOwner(uint8 mine, BattleGroundAVTeamIndex teamIdx)
{
    ...
    if (teamIdx != BG_AV_TEAM_NEUTRAL)
    {
        ...
        SendYell2ToAll(LANG_BG_AV_MINE_TAKEN , LANG_UNIVERSAL, 
                        GetSingleCreatureGuid(BG_AV_HERALD, (teamIdx == BG_AV_TEAM_ALLIANCE) ? BG_TEAM_HORDE:BG_TEAM_ALLIANCE),
...
void BattleGroundAV::EventPlayerDefendsPoint(Player* player, BG_AV_Nodes node)
{
    ...
    if (IsTower(node))
    {
        SendYell2ToAll(LANG_BG_AV_TOWER_DEFENDED, LANG_UNIVERSAL, 
                        GetSingleCreatureGuid(BG_AV_HERALD, (teamIdx == BG_TEAM_ALLIANCE) ? BG_TEAM_HORDE : BG_TEAM_ALLIANCE),
                        GetNodeName(node),
